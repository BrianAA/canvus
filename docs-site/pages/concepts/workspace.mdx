...
Custom CSS (Tailwind, brand fonts, imported template styles) is injected directly into the Shadow DOM so it is fully scoped and never pollutes the host application. For detail and examples, see [Scoped Stylesheets](/concepts/shadow-dom#scoped-stylesheets).

## JS Badge Marking

The SDK draws a visual `⚡️ JS` badge on the canvas when a node is flagged as containing JavaScript. The host application is responsible for detection. For detail and examples, see [Script Flagging Example](/concepts/canvas-overlay#script-flagging-example).
