...
export class NodeTree {
  private readonly nodes = new Map<string, ResolvedNode>();
  ...
  addNode(node: ResolvedNode, index?: number): void {
    if (this.nodes.has(node.id)) {
      throw new Error(
        `[NodeTree] Node "${node.id}" already exists.`,
      );
    }
    ...
    if (node.parentId !== null) {
      ...
      if (!parent) {
        throw new Error(
          `[NodeTree] Parent "${node.parentId}" does not exist.`,
        );
      }
  ...
  reparentNode(nodeId: string, newParentId: string | null, index?: number): void {
    ...
    if (!node) throw new Error(`[NodeTree] Node "${nodeId}" not found.`);
    ...
    if (newParentId !== null) {
      ...
      if (isAncestor(nodeId, newParentId, this.nodes)) {
        throw new Error(
          `[NodeTree] Cannot move "${nodeId}" into its own descendant "${newParentId}".`,
        );
      }
    }
  ...
  toDebugString(): string {
    ...
    const walk = (node: ResolvedNode, indent: number) => {
 ...
 */
function getDescendantIds(id: string, nodes: Map<string, ResolvedNode>): string[] {
  const result: string[] = [];
 ...
 */
export function computeAggregateBounds(
